...
const FETCH_GROUP_IMAGE = "groups/fetchGroupImage";
...
const fetchGroupImage = (image) => {
  return { type: FETCH_GROUP_IMAGE, payload: image };
};
...
export const getGroupImageThunk = (groupId) => async (dispatch) => {
  const response = await fetch(`/api/groups/${groupId}`);
  const resBody = await response.json();
  if (response.ok) dispatch(fetchOneGroup(resBody));
  return resBody;
